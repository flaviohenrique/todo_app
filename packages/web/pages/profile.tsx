...
const Profile = () => {
  ...
  const { status: loadingAddingStatus, error: addingAvatarError } =
    useAppSelector((state) => state.users.avatarAdding);
  ...
  return (
    <Flex my={2} borderWidth="1px">
      ...
      >
        <Box width="100%">
          <AvatarForm
            currentAvatarImgUrl={avatarImgUrl}
            onSelectAvatarFile={onSelectAvatarFileHandler}
            onSubmit={onSubmitHandler}
            isLoading={loadingAddingStatus == "loading"}
          ></AvatarForm>
        </Box>
        {selectedFile !== null ? <AvatarPreview file={selectedFile} /> : null}
      </Flex>
    </Flex>
  );
};
